...
export function CreateStoryForm() {
  ...
  async function go() {
    if (!premise.trim()) {
      setErr("The dig needs a spark. Write a premise to begin.");
      return;
    }
    setGenerating(true);
    setErr(null);
    try {
      ...
      if (!res.ok || !data.storyId) {
        throw new Error(data.error ?? "The scroll would not open. Try again.");
      }
      router.push(`/stories/${data.storyId}`);
    } catch (e) {
      setErr(e instanceof Error ? e.message : "The scroll would not open. Try again.");
      setGenerating(false);
    }
  }
      ...
      <div style={{ position: "relative", maxWidth: 660, margin: "0 auto", padding: "28px 20px 60px" }}>
        ...
        <div className="frame frame--papyrus" style={{ padding: "30px 30px 34px" }}>
          ...
          <p style={{ fontFamily: "var(--font-body)", fontSize: 16, color: "var(--muted)", marginBottom: 24 }}>
            Give the dig its first spark. Alexandria unrolls the opening passage — every page after is
            a fork.
          </p>
          ...
          <label className="label label--ink">Language</label>
