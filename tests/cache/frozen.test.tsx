...
describe("frozen cache — revisiting issues no generation request", () => {
  ...
  it("selecting an existing child calls onSelect without fetching", () => {
    ...
    render(
      <Reader storyId="s1" nodes={nodes} selectedId="r" onSelect={onSelect} onForked={vi.fn()} isDemo={false} isOwner={true} language="en" quotaRemaining={20} />,
    );
  ...
  it("forking — the write path — does hit the branch endpoint (guard discriminates)", async () => {
    render(
      <Reader storyId="s1" nodes={nodes} selectedId="r" onSelect={vi.fn()} onForked={vi.fn()} isDemo={false} isOwner={true} language="en" quotaRemaining={20} />,
    );
